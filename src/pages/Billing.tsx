...
export default function Billing() {
  ...
  const invoiceRows = useMemo(() => {
    return invoices.map((invoice) => {
      ...
      return {
        ...
        status: (
          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusClasses[statusKey] || statusClasses.open}`}>
            {statusLabel}
          </span>
        ),
  ...
  const paymentRows = useMemo(() => {
    return payments.map((payment) => ({
      ...
      status: (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusClasses.paid}`}>
          Posted
        </span>
      ),
    }))
  ...
  const scheduleRows = useMemo(() => {
    ...
    return Array.from(grouped.values()).map((entry) => {
      ...
      return {
        ...
        status: (
          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusClasses[statusKey] || statusClasses.active}`}>
  ...
  const taxRows = useMemo(() => {
    return taxTemplates.map((template) => ({
      ...
      status: (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${statusClasses.active}`}>
          Active
        </span>
      ),
      actions: (
        ...
        <div className="space-y-6">
          <div className="app-shell shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Open, paid, and overdue invoices across customers.</p>
              </div>
              <button
                className="text-sm font-medium text-primary-600 hover:text-primary-700"
                onClick={() => {
          ...
          <div className="app-shell shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">All posted payments, deposits, and adjustments.</p>
            </div>
            <div className="mt-4">
              <DataTable
          ...
          <div className="app-shell shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Recurring Schedules</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400">Contracts and subscriptions queued for automated billing.</p>
            </div>
            <div className="mt-4">
              <DataTable
          ...
          <div className="app-shell shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Taxes & Fees</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">Tax rules and service fees applied to invoices.</p>
              </div>
              <Link
                to="/tax-templates"
            ...
            <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-5">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Add tax / fee</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Create tax rules directly from Billing.</p>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-4">
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Country</label>
                  <select
              ...
              <label className="mt-3 inline-flex items-center text-xs text-gray-600 dark:text-gray-300">
                <input
                  type="checkbox"
            ...
            {taxError && (
              <div className="mt-3 rounded-md bg-red-50 dark:bg-red-900/20 p-3">
                <p className="text-sm text-red-800 dark:text-red-200">{taxError}</p>
              </div>
            )}
        ...
        <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Products, quantities, and pricing.</p>
            </div>
            {!editingInvoice && (
              <span className="text-xs text-gray-500 dark:text-gray-400">Save invoice to add items.</span>
            )}
          </div>
          ...
          {editingInvoice && (
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Product</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Qty</label>
                  <input
                    type="number"
                    min="1"
                    value={lineForm.qty}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Unit price</label>
                  <input
                    type="number"
                    min="0"
      ...
      >
        {selectedInvoice ? (
          <div className="space-y-4">
            ...
            <div>
              <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Line Items</h4>
              {linesLoading ? (
